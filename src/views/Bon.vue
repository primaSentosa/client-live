    ...
    <div class="overflow-auto"> 
        ...
        <v-simple-table
            ...
            <tbody>
                ...
                <td>      
                    <b-badge variant="danger" class="lg" v-if="item.metode === 'Bon'" @click.prevent="ubah(item._id)" style="cursor:pointer;">BON</b-badge>    
                </td>
      ...
      methods:{     
          ...
          getRp(text){
            ...
            fetchAllBon(){
                ...
                this.loading = true
                axios({
                    url: `${this.$store.state.url}/transaksi/penjualan/status?mulai=${this.date[0]}&akhir=${this.date[1]}&metode=Bon`,
                    method: 'get',
                    headers:{
            ...
            ubah(id){
                Swal.fire({
                    title: 'Apakah customer sudah melunasi bon?',
                    showDenyButton: true,
                    ...
                    if (result.isConfirmed) {
                        this.loading = true
                        axios({
                            url: `${this.$store.state.url}/transaksi/metode/${id}?metod=Tunai`,
                            method: 'put',
                            headers:{
                                token : localStorage.getItem('token')
                            }
